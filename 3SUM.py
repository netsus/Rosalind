...
문제 : 20이하의 수(k)와 10,000이하의 수 n이 주어지고, 다음 줄부터 n개의 수로 이루어진 배열이 k개 주어진다. (행 구분자)
    -> n개의 수로 이루어진 각 배열에 대해 3개의 수를 모두 더했을때 0이 되는 3개의 수가 있을때, 차례대로 3개의 수의 인덱스를 출력(첫번쨰가 1)
    만약 주어진 배열에서 3개의 수를 모두 더해도 0이 되지않으면 -1을 출력.
    ex) n이 3이고 1 3 5 -4 -> 이렇게 주어지면 1 3 -4를 더했을때 0이되므로, 출력은 1 2 4가 되어야 한다.(1번쨰, 2번째, 4번째 더했을때 0 되기때문)
    ...
    for 문으로 리스트 개수(n개)만큼 반복하며, (0부터 n미만 -> a)
        ...
        또, for문으로 a+1부터 n 미만(-> b)을 반복하며
            ...
            first와 second값을 더한 것에 -1 곱한 값이 found_dic의 키값에 존재한다면:
                ...
                found=True로 바꾸고 이중 포문을 빠져나가야 하기 때문에 break를 두 번 한다. (line46에서 한번 line36에서 한번)
